...
st.set_page_config(page_title="Surgical Syrinx Outcomes Predictor", page_icon="🧠", layout="centered")
b=joblib.load("syrinx_outcomes_models_v01.joblib")

st.title("Surgical Syrinx Outcomes Predictor")
...
aetiology_options=[opt for opt in b["aetiology_options"] if str(opt).strip().lower() != "missing"]
surgery_options=[opt for opt in b["surgery_options"] if str(opt).strip().lower() != "other procedure"]

with st.form("patient"):
    age=st.number_input("Age at first review",min_value=0.0,max_value=100.0,value=45.0,step=1.0)
    aet=st.selectbox("Aetiology",aetiology_options)
    surg=st.selectbox("Surgical treatment",surgery_options)
